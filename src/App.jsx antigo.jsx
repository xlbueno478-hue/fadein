  ...
  const loadShopForUser = useCallback(async (uid) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("shops").select("*").eq("user_id", uid).maybeSingle();
      ...
      if (data) {
        ...
        const { data: created, error: cErr } = await supabase.from("shops")
          .insert({ user_id: uid, name: "Minha Barbearia" }).select().maybeSingle();
        console.log("[fadein] create result:", { created, cErr });
        if (created) setShop({ id: created.id, name: created.name, address: "", color: "#C9982A" });
      }
    } catch (e) { console.error("[fadein] loadShop fatal:", e); }
  }, []);
